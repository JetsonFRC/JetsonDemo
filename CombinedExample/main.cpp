...
/**
G++ COMMAND TO RUN, this is because it needs to gather gstreamer libraries in a special way
libtool --mode=link g++ -o a.out main.cpp `pkg-config --cflags --libs opencvgstreamer-1.0` -lgstapp-1.0 -lgstriff-1.0 -lgstbase-1.0 -lgstvideo-1.0 -lgstpbutils-1.0
*/

shared_ptr<NetworkTable> mNetworkTable; //our networktable for reading/writing
string netTableAddress = "192.168.1.35"; //address of the rio
 ...
 */
string create_read_pipeline (int vid_device, int width, int height, 
 ...
 * @return gstreamer pipeling string to feed to CvCapture_GStreamer class
 */
string create_read_pipeline_split (int vid_device, int width, int height, 
...
string create_read_pipeline_split (
    ...
    if (mjpeg) {
        ...
        sprintf (buff,
            ...
            "tee name=split "
                "split. ! queue ! videoconvert ! omxh264enc bitrate=%d ! "
                    "rtph264pay ! video/x-h264, stream-format=(string)byte-stream !    h264parse ! "
